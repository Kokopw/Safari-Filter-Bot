...
from utils import get_settings, get_size, is_subscribed, is_req_subscribed, temp, get_shortlink, get_seconds, safe_delete
...
async def save_group_settings(group_id, key, value):
    ...
    try:
        await db.save_group_setting(group_id, key, value)
        return True
    except Exception as e:
        logging.error(f"Error saving group setting {key} for group {group_id}: {e}")
        return False
...
async def get_settings(group_id):
    ...
    try:
        ...
        logging.error(f"Error getting settings for group {group_id}: {e}")
        return None
    ...
    try: 
        ...
        if len(safari.command) == 2:
            if safari.command[1].startswith(('verify', 'sendall')):
                ...
                await safe_delete(dlt, delay=600)
                return
        ...
        if not await db.has_premium_access(user_id):
            ...
            if (not user_verified or is_second_shortener or is_third_shortener) and is_verify:
                ...
                await asyncio.sleep(600) 
                await safe_delete(d, delay=600)
                await safe_delete(safari)
                return
...
async def delete(bot, message):
    ...
    if result.deleted_count:
        ...
        if result.deleted_count:
            ...
            # files indexed before https://github.com/EvamariaTG/EvaMaria/commit/f3d2a1bcb155faf44178e5d7a685a1b533e714bf#diff-86b613edf1748372103e94cacff3b578b36b698ef9c16817bb98fe9ef669R39 
...
async def set_log(client, message):
    ...
    await safe_delete(sts, delay=1.2)
    try:
...
async def group_details(client, message):
    grp_id = message.chat.id
    title = message.chat.title
    
    try:
        user = await client.get_chat_member(grp_id, message.from_user.id)
        is_admin = user.status in [enums.ChatMemberStatus.ADMINISTRATOR, enums.ChatMemberStatus.OWNER] or str(message.from_user.id) in ADMINS
        if not is_admin:
            return await message.reply_text("<b>❌ You must be an admin to use this command!</b>")
        ...
        buttons = [
            [
                InlineKeyboardButton("♻️ ʀᴇsᴇᴛ ᴅᴀᴛᴀ ♻️", callback_data=f"reset_group_settings#{grp_id}"),
                InlineKeyboardButton("❌ ᴄʟᴏsᴇ ❌", callback_data="close_data")
            ]
        ]
        reply_markup = InlineKeyboardMarkup(buttons)

        details_text = f"""<b>⚙️ ʏᴏᴜʀ sᴇᴛᴛɪɴɢs ꜰᴏʀ - {title} 🔥</b>

<b>✅️ 1sᴛ ᴠᴇʀɪꜰʏ sʜᴏʀᴛɴᴇʀ ɴᴀᴍᴇ/ᴀᴘɪ</b>
ɴᴀᴍᴇ - <code>{settings.get("verify", VERIFY_URL)}</code>
ᴀᴘɪ - <code>{settings.get("verify_api", VERIFY_API)}</code>

<b>✅️ 2ɴᴅ ᴠᴇʀɪꜰʏ sʜᴏʀᴛɴᴇʀ ɴᴀᴍᴇ/ᴀᴘɪ</b>
ɴᴀᴍᴇ - <code>{settings.get("verify_2", VERIFY_URL2)}</code>
ᴀᴘɪ - <code>{settings.get("verify_api2", VERIFY_API2)}</code>

<b>✅️ 3ʀᴅ ᴠᴇʀɪꜰʏ sʜᴏʀᴛɴᴇʀ ɴᴀᴍᴇ/ᴀᴘɪ</b>
ɴᴀᴍᴇ - <code>{settings.get("verify_3", VERIFY_URL3)}</code>
ᴀᴘɪ - <code>{settings.get("verify_api3", VERIFY_API3)}</code>

<b>🧭 𝟸ɴᴅ ᴠᴇʀɪꜰɪᴄᴀᴛɪᴏɴ ᴛɪᴍᴇ -</b> <code>{settings.get("verify_time", TWO_VERIFY_GAP)}</code>

<b>🧭 𝟹ʀᴅ ᴠᴇʀɪꜰɪᴄᴀᴛɪᴏɴ ᴛɪᴍᴇ -</b> <code>{settings.get("verify_time2", THIRD_VERIFY_GAP)}</code>

<b>📝 ʟᴏɢ ᴄʜᴀɴɴᴇʟ ɪᴅ -</b> <code>{settings.get("log", LOG_CHANNEL)}</code>

<b>🌀 ꜰꜱᴜʙ ᴄʜᴀɴɴᴇʟ ɪᴅ -</b> <code>{settings.get("fsub_id", AUTH_CHANNEL)}</code>

<b>📍1 ᴛᴜᴛᴏʀɪᴀʟ ʟɪɴᴋ -</b> {settings.get("tutorial", TUTORIAL)}

<b>📍2 ᴛᴜᴛᴏʀɪᴀʟ ʟɪɴᴋ -</b> {settings.get("tutorial2", TUTORIAL2)}

<b>📍3 ᴛᴜᴛᴏʀɪᴀʟ ʟɪɴᴋ -</b> {settings.get("tutorial3", TUTORIAL3)}

<b>🎯 ɪᴍᴅʙ ᴛᴇᴍᴘʟᴀᴛᴇ -</b> 
<code>{settings.get("template", IMDB_TEMPLATE)}</code>

<b>📂 ꜰɪʟᴇ ᴄᴀᴘᴛɪᴏɴ -</b> 
<code>{settings.get("caption", CUSTOM_FILE_CAPTION)}</code>

<b>𝖩𝗈𝗂𝗇➥ 「𝘾𝙝𝙖𝙧𝙡𝙞𝙚 𝙈𝙤𝙫𝙞𝙚𝙨」</b>"""

        await message.reply_text(details_text, parse_mode=enums.ParseMode.HTML, disable_web_page_preview=True, reply_markup=reply_markup)
        
    except Exception as e:
        await message.reply_text(f"<b>❌ Error fetching group details:</b> <code>{str(e)}</code>")

@Client.on_message(filters.command('stats') & filters.user(ADMINS))
async def stats(bot, message):
    """Show bot statistics including database status"""
    msg = await message.reply('<b>Fᴇᴛᴄʜɪɴɢ sᴛᴀᴛs...</b>')
    ...
    try:
        ...
        active_db, primary_size, secondary_size = await db.get_active_db()
        
        # Format database sizes
        primary_size_str = get_size(db_stats["primary"]["size"])
        secondary_size_str = get_size(db_stats["secondary"]["size"]) if db.has_secondary else "N/A"
        max_size_str = get_size(DB_MAX_SIZE)
        
        stats_text = f"""<b>📊 BOT STATISTICS</b>

<b>👥 Users:</b> <code>{total_users}</code>
<b>💬 Chats:</b> <code>{total_chats}</code>
<b>📁 Files:</b> <code>{total_files}</code>

<b>🗄️ DATABASE STATUS</b>

<b>📊 Primary Database:</b>
├ <b>Size:</b> <code>{primary_size_str} / {max_size_str}</code>
├ <b>Collections:</b> <code>{db_stats["primary"]["collections"]}</code>
├ <b>Objects:</b> <code>{db_stats["primary"]["objects"]}</code>
└ <b>Status:</b> <code>{db_stats["primary"]["status"]}</code>

<b>📊 Secondary Database:</b>
├ <b>Size:</b> <code>{secondary_size_str} / {max_size_str if db.has_secondary else "Not Configured"}</code>
├ <b>Collections:</b> <code>{db_stats["secondary"]["collections"]}</code>
├ <b>Objects:</b> <code>{db_stats["secondary"]["objects"]}</code>
└ <b>Status:</b> <code>{db_stats["secondary"]["status"]}</code>

<b>⚡ Active Database:</b> <code>{active_db.title()}</code>
<b>🔄 Auto Switch:</b> <code>{"Enabled" if AUTO_SWITCH_DB else "Disabled"}</code>"""

        await msg.edit(stats_text, parse_mode=enums.ParseMode.HTML)
        
    except Exception as e:
        await msg.edit(f"<b>Error fetching stats:</b> <code>{str(e)}</code>")

@Client.on_message(filters.command('switch_db') & filters.user(ADMINS))
async def switch_database(bot, message):
    """Manually switch between databases"""
    if not db.has_secondary:
        return await message.reply("<b>❌ Secondary database is not configured!</b>")
    
    try:
        args = message.text.split()
        if len(args) != 2 or args[1] not in ['primary', 'secondary']:
            return await message.reply("<b>Usage:</b> <code>/switch_db primary</code> or <code>/switch_db secondary</code>")
        
        target_db = args[1]
        active_db, _, _ = await db.get_active_db()
        
        if active_db == target_db:
            return await message.reply(f"<b>ℹ️ {target_db.title()} database is already active!</b>")
        
        # Implementation depends on db.switch_to_db or similar method
        # For now, we'll inform that it's requested
        await message.reply(f"<b>🔄 Database switch requested to {target_db.title()}</b>\n\n<b>Note:</b> Manual switching will be overridden by auto-switch if enabled.")
        
    except Exception as e:
        await message.reply(f"<b>❌ Error switching database:</b> <code>{str(e)}</code>")
...
🎯 ɪᴍᴅʙ ᴛᴇᴍᴘʟᴀᴛᴇ - `{settings.get('template', IMDB_TEMPLATE)}`"""
    ...
    await safe_delete(dlt, delay=300)
...
async def clear_database(client, message):
    if len(message.command) != 2 or message.command[1] != "confirm":
        return await message.reply_text(
            "<b>⚠️ WARNING: This will completely reset both databases!</b>\n\n"
            "This includes all users, groups, premium data, and indexed files.\n\n"
            "To proceed, use: <code>/cleardb confirm</code>"
        )
    
    msg = await message.reply_text("<b>🗑️ Clearing databases... Please wait.</b>")
    try:
        success = await db.clear_db()
        if success:
            await msg.edit("<b>✅ Both databases have been completely cleared and reset!</b>")
            logging.info(f"Database wipe executed by admin: {message.from_user.id}")
        else:
            await msg.edit("<b>❌ Failed to clear databases. Check logs for details.</b>")
    except Exception as e:
        await msg.edit(f"<b>❌ An error occurred:</b> <code>{str(e)}</code>")
