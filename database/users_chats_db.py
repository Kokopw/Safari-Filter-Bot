...
VERIFY_URL2, VERIFY_API2, VERIFY_URL3, VERIFY_API3, LIMIT_MODE, TWO_VERIFY_GAP, THIRD_VERIFY_GAP, STREAM_MODE, LOG_CHANNEL, IS_VERIFY, AUTH_CHANNEL, CUSTOM_FILE_CAPTION, FILE_LIMITE, SEND_ALL_LIMITE, STREAM_SITE, STREAM_API, COLLECTION_NAME
...
class Database:
    ...
    def __init__(self, uri, database_name, uri_2=None, database_name_2=None):
        ...
        # DB 1: User data
        self._client1 = motor.motor_asyncio.AsyncIOMotorClient(self.db1_uri)
        self.db1 = self._client1[self.db1_name]
        
        # DB 2: Files data (if available, otherwise use DB 1)
        if self.db2_uri and self.db2_name:
            self._client2 = motor.motor_asyncio.AsyncIOMotorClient(self.db2_uri)
            self.db2 = self._client2[self.db2_name]
        else:
            self._client2 = self._client1
            self.db2 = self.db1

        # Collections in DB 1 (User data)
        self.col = self.db1.users
        self.grp = self.db1.groups
        self.users = self.db1.uersz
        self.codes = self.db1.codes
        self.safari = self.db1.safari
        self.req = self.db1.requests
        self.links_col = self.db1.links
        self.verify_id = self.db1.verify_id
        self.settings_col = self.db1.settings

        # Collections in DB 2 (File data)
        self.files_col = self.db2[COLLECTION_NAME]

    async def switch_db(self):
        # Database switching disabled: System now uses strict data separation
        return False

    async def check_db_size(self):
        try:
            db1_stats = await self._client1[self.db1_name].command("dbstats")
            return db1_stats['dataSize']
    ...
    async def clear_db(self):
        """Completely clear all collections in DB1 and DB2"""
        try:
            # Clear DB1 collections
            await self.col.delete_many({})
            await self.grp.delete_many({})
            await self.users.delete_many({})
            await self.codes.delete_many({})
            await self.safari.delete_many({})
            await self.req.delete_many({})
            await self.links_col.delete_many({})
            await self.verify_id.delete_many({})
            await self.settings_col.delete_many({})
            
            # Clear DB2 collections
            await self.files_col.delete_many({})
            
            # Clear other linked collections if they share the same DB
            # CONNECTION, referusers, refers (from other files)
            await self.db1['CONNECTION'].delete_many({})
            await self.db1['referusers'].delete_many({})
            await self.db1['refers'].delete_many({})
    ...
    async def get_db_size(self):
        try:
            db1_stats = await self._client1[self.db1_name].command("dbstats")
            db2_stats = await self._client2[self.db2_name].command("dbstats")
            return db1_stats['dataSize'], db2_stats['dataSize']
        except Exception as e:
    ...
    async def get_expired(self, current_time):
        expired_users = []
        if data := await self.users.find({"expiry_time": {"$lt": current_time}}).to_list(length=None):
            for user in data:
    ...
    async def get_db_status(self):
        try:
            ...
            return {
                ...
                "db1_size": db1_stats.get('dataSize', 0),
                "db2_name": self.db2_name,
                "db2_status": "Connected" if self.db2_uri else "Linked to DB1",
                "db2_size": db2_stats.get('dataSize', 0),
            }
        except Exception as e:
            logging.error(f"Error getting DB status: {e}")
            return {}
    ...
    async def get_db_stats(self):
        ...
        try:
            ...
            primary_stats = await self._client1[self.db1_name].command("dbstats")
            primary_data = {
            ...
            return {
                ...
                "secondary": {"size": 0, "collections": 0, "objects": 0, "status": "Error"}
            }

    async def get_active_db(self):
        """Get active database names and sizes"""
        try:
            db1_stats = await self._client1[self.db1_name].command("dbstats")
            db2_stats = await self._client2[self.db2_name].command("dbstats")
            return "split_mode", db1_stats.get('dataSize', 0), db2_stats.get('dataSize', 0)
        except Exception as e:
            logging.error(f"Error getting active DB info: {e}")
            return "error", 0, 0
